...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 4 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 1 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\coupon-redemptions](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\coupon-redemptions)
...
graph LR
  ...
  coupon_redemptions["coupon_redemptions"]:::center
  coupon_redemptions -->|FK| coupons["coupons"]:::link
  coupon_redemptions -->|FK| orders["orders"]:::accent
  coupon_redemptions -->|FK| tenants["tenants"]:::link
  coupon_redemptions -->|FK| users["users"]:::accent
...
- Outbound (depends on): "coupons", "orders", "tenants", "users"
...
- `redeemed_at` – default=CURRENT_TIMESTAMP(6)
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package coupon-redemptions -Apply
