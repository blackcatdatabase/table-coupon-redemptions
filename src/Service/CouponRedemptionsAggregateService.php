...
namespace BlackCat\Database\Packages\CouponRedemptions\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto;
use BlackCat\Database\Packages\CouponRedemptions\Mapper\CouponRedemptionDtoMapper;
 ...
 */
final class CouponRedemptionsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CouponRedemptionRepository $couponRedemptionRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'coupon_redemptions' => $this->couponRedemptionRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\CouponRedemptions\ModuleException("Record {$idStr} not found for locking.");
            }
