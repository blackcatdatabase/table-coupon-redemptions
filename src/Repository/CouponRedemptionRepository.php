...
use BlackCat\Database\Packages\CouponRedemptions\Repository\CouponRedemptionRepositoryInterface;
...
class CouponRedemptionRepository implements CouponRedemptionRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\CouponRedemptions\Definitions::class);
    ...
    public function getByTenantIdAndOrderIdAndCouponId(int $tenantId, int $orderId, int $couponId, bool $asDto = false): array|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'order_id' => $orderId, 'coupon_id' => $couponId ], $asDto);
    }
    ...
    public function getIdByTenantIdAndOrderIdAndCouponId(int $tenantId, int $orderId, int $couponId) {
        $row = $this->getByTenantIdAndOrderIdAndCouponId($tenantId, $orderId, $couponId, false);
