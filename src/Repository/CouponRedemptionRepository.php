...
namespace BlackCat\Database\Packages\CouponRedemptions\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\CouponRedemptions\Definitions;
use BlackCat\Database\Packages\CouponRedemptions\Criteria;
use BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto as Dto;
use BlackCat\Database\Packages\CouponRedemptions\Mapper\CouponRedemptionDtoMapper as RowMapper;
...
final class CouponRedemptionRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\CouponRedemptions\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\CouponRedemptions\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null */
    public function getByTenantIdAndOrderIdAndCouponId(int $tenantId, int $orderId, int $couponId, bool $asDto = false): array|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'order_id' => $orderId, 'coupon_id' => $couponId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\CouponRedemptions\Mapper\CouponRedemptionDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndOrderIdAndCouponId(int $tenantId, int $orderId, int $couponId): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'order_id') . ' = :uniq_order_id' . ' AND ' . 't.' . Ident::q($this->db, 'coupon_id') . ' = :uniq_coupon_id';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_order_id' => $orderId, 'uniq_coupon_id' => $couponId ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndOrderIdAndCouponId(int $tenantId, int $orderId, int $couponId) {
        $row = $this->getByTenantIdAndOrderIdAndCouponId($tenantId, $orderId, $couponId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\CouponRedemptions\Dto\CouponRedemptionDto|null {
